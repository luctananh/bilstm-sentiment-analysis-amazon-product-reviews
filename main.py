...
#Chuẩn bị văn bản thô cho các tác vụ NLP bằng cách: Chuẩn hóa định dạng ,Giảm nhiễu , Giảm độ phức tạp của dữ liệu
...
# Huấn luyện mô hình Word2Vec để tạo vector biểu diễn từ (word embeddings) từ dữ liệu văn bản, giúp máy tính hiểu được ngữ nghĩa và mối quan hệ giữa các từ.
...
class BiLSTMModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, lstm_units, num_classes, embedding_matrix, max_len):
        ...
        # 1. Lớp Embedding với trọng số cố định
        self.embedding = nn.Embedding(vocab_size, embedding_dim)
        self.embedding.weight = nn.Parameter(torch.tensor(embedding_matrix, dtype=torch.float32))
        self.embedding.weight.requires_grad = False  # Không cập nhật trọng số embedding
        # 2. Lớp Bi-directional LSTM,2 lớp LSTM xếp chồng,Input format: [batch, seq_len, features], LSTM 2 chiều, Regularization
    ...
    def forward(self, x):
        ...
        output, (hn, cn) = self.lstm(x)  # hn: hidden states của tất cả các lớp
        # Kết hợp hidden states từ 2 chiều
        forward_hidden = hn[-2, :, :]   #- chiều forward
        backward_hidden = hn[-1, :, :]  #- chiều backward
        hidden = torch.cat((forward_hidden, backward_hidden), dim=1)
        hidden = self.dropout(hidden)
        out = self.fc(hidden) # Dự đoán lớp
        return out
...
def train_and_evaluate_bilstm(tweets, embedding_dim=100, lstm_units=128, dropout_rate=0.2, max_len=50,
    ...
    # Tokenize từng câu thành danh sách các từ
    tokenized_sentences = [text.split() for text in processed_texts]
    
    # Tạo bigrams: kết hợp các từ đơn và cặp từ (bigram)
    phrases = Phrases(tokenized_sentences, min_count=1, threshold=2)  # threshold thấp giúp tạo nhiều bigram
    bigram = Phraser(phrases)
    tokenized_sentences_with_bigrams = [bigram[sentence] for sentence in tokenized_sentences]
    
    # Huấn luyện Word2Vec sử dụng dữ liệu đã có cả unigrams và bigrams
    word2vec_model = train_word2vec_model(tokenized_sentences_with_bigrams, vector_size=embedding_dim)
    ...
    # Sử dụng Keras Tokenizer cho bước chuyển văn bản thành chuỗi số
